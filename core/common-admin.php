...
function loginActions( ){
  global $config, $lang;
    $admin_login = isset($_GET['poka']) ? $config['login_email'] : null;
    $admin_pass = isset($_GET['poka']) ? $config['login_pass'] : ""; 
  ...
  if( !isset( $_SESSION[$config['session_key_name']] ) || !is_int( $_SESSION[$config['session_key_name']] ) ){
    ...
    if( isset( $config['failed_logs'] ) && isset( $config['failed_login_time'] ) && $config['failed_logs'] > 2 && time( ) - $config['failed_login_time'] <= 900 ){
      ...
      if( $_GET['p'] == 'login' && isset( $_POST['sEmail'] ) && checkEmail( $_POST['sEmail'] ) && !empty( $_POST['sPass'] ) ){
        ...
        $content = '<form method="post" action="?p=login" id="login-form" class="login-form">
           
             <h5>Panel administracyjny - '.( isset( $bFirstLog ) ? $lang['Type_login_password'] : $lang['log_in'] ).'</h5>
             
             
            <div class="forms full mt-2">
            
            <div class="form-floating form-item">
                <input type="email" class="form-control input" id="sEmail" name="sEmail" value="'.( isset( $_COOKIE['sEmail'] ) ? strip_tags( $_COOKIE['sEmail'] ) : $admin_login ).'"  placeholder="'.$lang['Email'].'" data-form-check="email">
                <label for="name">'.$lang['Email'].'</label>
            </div>
            
            <div class="form-floating form-item">
        <input type="'.( isset( $bFirstLog ) ? 'text' : 'password' ).'"  class="form-control" id="sPass" name="sPass" value="'.$admin_pass.'"   placeholder="'.$lang['Password'].'" required data-form-check="required">
        <label for="name">'.$lang['Password'].'</label>
              ...
              <input type="hidden" name="bAcceptLicense" value="1" />
              '.getYesNoBox( 'iAcceptLicense', 0, Array( 'value' => 'true', 'data-form-check' => "required", 'data-form-msg' => ( $config['admin_lang'] == 'pl' ? 'Zaakceptuj licencję' : 'Accept the license' ) ) ).'<label for="iAcceptLicense" class="form-check-label">'.( $config['admin_lang'] == 'pl' ? 'Akceptuję <a href="http://opensolution.org/licencje.html?notice=" target="_blank">licencję systemu Quick.Cms &raquo;</a>' : 'I accept <a href="http://opensolution.org/licenses.html?notice=" target="_blank">Quick.Cms license &raquo;</a>' ).'</label><span class="label">&nbsp;</span></div>' : null ).
              '
              
              <div class="form-button d-flex justify-content-between">
                <input type="submit" class="button main" value="'.$lang['log_in'].'" />
                 <a href="mailto:konrad@kosiorski.pl" target="_blank" class="link_underline">'.$lang['forgot_your_password'].'</a>
                </div>
    ...
    echo '<div class="login-wrapper"><section class="login-panel" id="login-panel"'.( isset( $bFirstLog ) ? ' class="init"' : null ).'>
    
      <header class="header">
        <a href="https://kosa-x.com/" target="_blank"><img src="templates/admin/img/logo-kosax.png" alt="KOSA X">
        <span class="content"><strong>Digital marketing</strong><br>kosa-x.com</span></a>
      </header>
      '.$content.'
      <footer class="footer d-flex justify-content-between">
        <a href="./" class="link_underline">'.$lang['homepage'].' <strong>'.$config['logo'].'</strong></a>
        <a href="http://opensolution.org/" target="_blank">Quick.Cms v'.$config['version'].'</a>
   
        
      
      </footer>
    </section></div><style>.mainFooter{display:none;}</style>';
    ...
    for( $i = 0; $i < $iCount; $i++ ){
      foreach( $aForm as $sKey => $sValue ){
        if( preg_match( '/'.$sVariable."\['".$sKey."'\]".' = /', $aFile[$i] ) && strstr( $aFile[$i], '=' ) ){
          ...
          $sValue = changeSpecialChars( trim( str_replace( Array( '\\', '"', "\n", "\r" ), Array( '&#92;', '&quot;', '\n', '' ), $sValue ) ) );
