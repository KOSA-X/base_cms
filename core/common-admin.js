...
function displayFilesOptions( oObj ){
  ...
  if( oObj.checked === true ){
    ...
    $( '#fileTr'+iFile+' .default' ).empty( );
  }
  if( iSize > 0 && oObj.checked === true ){
    $( '#fileTr'+iFile+' .default' ).html( '<div class="mb-1"><input type="radio" name="iDirDefaultImage" value="'+iFile+'" class="mr-1" id="file-'+iFile+'"/><label for="file-'+iFile+'">Główna miniatura</label></div>' );
    $( '#fileTr'+iFile+' .location' ).html( '<select name="aDirFilesTypes['+iFile+']" class="form-control">'+sTypesSelect+'</select>' );
    $( '#fileTr'+iFile+' .thumb' ).html( '<select name="aDirFilesSizes['+iFile+']" class="form-control">'+sSizeSelect+'</select>' );
  }
  $( '#files-dir-table' ).fixedfiles();
}
